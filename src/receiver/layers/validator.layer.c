...
void LRP_ReceiverValidatorLayer_handler(LRPReceiverSessionProvider *const sessionProvider) {
    if (sessionProvider->validatorCurrentFrame->status != RECEIVER_FRAME_READY_TO_CHECK) {
        return;
    }
    ...
    LRP_Frame_setStatus(sessionProvider->validatorCurrentFrame, RECEIVER_FRAME_READY_TO_READ);
